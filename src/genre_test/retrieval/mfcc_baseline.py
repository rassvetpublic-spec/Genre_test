...
FEATURE_POLICY = (
    f"mfcc{N_MFCC}-dct{MFCC_DCT_TYPE}-{MFCC_NORM}-lifter{MFCC_LIFTER}-"
    f"mels{N_MELS}-power{MFCC_POWER:g}-chroma{N_CHROMA}-"
    f"contrast{N_CONTRAST_BANDS + 1}-fmin{CONTRAST_FMIN:g}-"
    f"quantile{CONTRAST_QUANTILE:g}-linear{int(CONTRAST_LINEAR)}-"
    f"meanstd-sr{SAMPLE_RATE}-mono-nfft{N_FFT}-hop{HOP_LENGTH}"
)
...
PREPROCESSING_VERSION = (
    f"{FEATURE_POLICY}-rms{TARGET_RMS:g}-minrms{MIN_RMS_DBFS:g}dbfs-familyl2equal-"
