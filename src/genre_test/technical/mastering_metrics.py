...
def estimate_alignment(
    reference: np.ndarray, candidate: np.ndarray, sample_rate: int, max_lag_seconds: float = 2.0
) -> dict[str, Any]:
    analysis_rate = 1000
...
def align_pair(
    ...
    return reference[ref_start : ref_start + count, :channels], candidate[
        cand_start : cand_start + count, :channels
    ]
...
def detect_transient_events(
    reference: np.ndarray, sample_rate: int, max_events: int = 64
    ...
    energy = y * y
    fast = np.sqrt(
        np.maximum(uniform_filter1d(energy, size=max(1, int(0.008 * analysis_rate))), 1e-18)
    )
    slow = np.sqrt(
        np.maximum(uniform_filter1d(energy, size=max(2, int(0.080 * analysis_rate))), 1e-18)
    )
    ...
    peaks, _ = find_peaks(
        score, distance=max(1, int(0.085 * analysis_rate)), height=height, prominence=0.75
    )
    peaks = peaks[fast_db[peaks] >= floor]
    margin = int(0.180 * analysis_rate)
    peaks = peaks[(peaks >= margin) & (peaks < score.size - margin)]
    ranked = sorted(
        peaks.tolist(), key=lambda i: (float(score[i]), float(fast_db[i])), reverse=True
    )[:max_events]
...
def measure_transient_retention(
    ...
    for number, event in enumerate(events, 1):
        ...
        if a0 < 0 or s1 > reference.shape[0]:
            continue
        rp, cp = (
            _db_amplitude(float(np.max(np.abs(reference[a0:a1])))),
            _db_amplitude(float(np.max(np.abs(candidate[a0:a1])))),
        )
        ...
        rows.append(
            {
                ...
                "attack_peak_delta_db": cp - rp,
                "attack_rms_delta_db": rms_delta,
    ...
    return {
        "status": status,
        "events_detected": detector.get("events_detected", len(events)),
        "events_used": len(rows),
        "attack_guard_delta_db": guard,
        ...
        "windows_ms": {"attack": [-5, 35], "sustain": [45, 145]},
        "interpretation": "Negative delta means less event attack after active-RMS matching.",
        "algorithm_id": ALGORITHM_ID,
    }, rows
...
def overall_mono_retention(audio: np.ndarray) -> float | None:
    ...
    mid = 0.5 * (left + right)
    return _mono_retention(
        float(np.mean(left * left)), float(np.mean(right * right)), float(np.mean(mid * mid))
    )
...
def _band_retention(audio: np.ndarray, sample_rate: int) -> dict[str, float | None]:
    ...
    for name, low, high in MONO_BANDS:
        ...
        if np.any(mask):
            out[name] = _mono_retention(
                float(trapezoid(lp[mask], f[mask])),
                float(trapezoid(rp[mask], f[mask])),
                float(trapezoid(mp[mask], f[mask])),
            )
    return out
...
def measure_mono_loss(
    ...
    overall_delta = (
        None if ref_overall is None or cand_overall is None else cand_overall - ref_overall
    )
    ref_bands, cand_bands = (
        _band_retention(reference, sample_rate),
        _band_retention(candidate, sample_rate),
    )
    bands, deltas = {}, []
    ...
    for name, low, high in MONO_BANDS:
        if (
            name in ref_bands
            and name in cand_bands
            and ref_bands[name] is not None
            and cand_bands[name] is not None
        ):
            delta = float(cand_bands[name] - ref_bands[name])
            deltas.append((name, delta))
            bands[name] = {
                "low_hz": low,
                "high_hz": min(high, sample_rate * 0.5),
                "reference_mono_retention_db": ref_bands[name],
                "candidate_mono_retention_db": cand_bands[name],
    ...
    return {
        "status": status,
        "definition": "10*log10(power((L+R)/2) / mean(power(L), power(R)))",
        "overall": {
            ...
            "candidate_lr_correlation": correlation_lr(candidate),
        },
        "bands": bands,
        "events_considered": len(event_samples),
        "worst_scope": worst_scope,
        "worst_candidate_minus_reference_db": worst,
        "warning_threshold_db": warn_db,
        "fail_threshold_db": fail_db,
        "interpretation": "Negative candidate-minus-reference means extra loss on mono collapse.",
        "algorithm_id": ALGORITHM_ID,
    }
...
def measure_codecs(
    ...
    codecs: Sequence[str],
    *,
    target_dbtp: float | None = None,
    safety_margin_db: float = 0.1,
) -> dict[str, Any]:
    if not codecs:
    ...
    with tempfile.TemporaryDirectory(prefix="genre_test_codec_") as tmp:
        ...
        for name in codecs:
            ...
            trim = (
                None
                if target_dbtp is None or peak is None
                else max(0.0, peak - target_dbtp + safety_margin_db)
            )
            status = (
                "MEASURED"
                if target_dbtp is None and peak is not None
                else "FAIL"
                if peak is None or (trim or 0.0) > 0.0
                else "PASS"
            )
            rows.append(
                {
                    ...
                    "label": spec["label"],
                    "status": status,
                    "decoded_true_peak_dbtp": peak,
                    "decoded_integrated_lufs": stats["integrated_lufs"],
    ...
    return {
        "status": max((r["status"] for r in rows), key=lambda s: STATUS_RANK.get(s, 0)),
        "results": rows,
        "algorithm_id": ALGORITHM_ID,
    }


def compare_mastering_arrays(
    reference: np.ndarray,
    candidate: np.ndarray,
    sample_rate: int,
    *,
    ...
    max_events: int = 64,
    attack_warn_db: float = -0.75,
    attack_fail_db: float = -1.5,
    mono_warn_db: float = -0.5,
    mono_fail_db: float = -1.5,
) -> dict[str, Any]:
    ...
    transient, event_rows = measure_transient_retention(
        ref, cand, sample_rate, events, detector, attack_warn_db, attack_fail_db
    )
    ...
    return {
        ...
        "transient_retention": transient,
        "transient_events": event_rows,
        "mono_loss": mono_loss,
        "overall_status": overall,
    }


def compare_mastering_files(
    reference_path: Path,
    candidate_path: Path,
    *,
    codecs: Sequence[str] = (),
    target_dbtp: float | None = None,
    codec_safety_margin_db: float = 0.1,
    **kwargs: Any,
) -> dict[str, Any]:
    ...
    result["reference"] = {
        ...
        "load_backend": ref_backend,
    }
    result["candidate"] = {
        "path": str(candidate_path),
        "source_sample_rate": cand_rate,
        "analysis_sample_rate": ref_rate,
        "channels": int(candidate.shape[1]),
        "load_backend": cand_backend,
    }
    codec = measure_codecs(
        candidate_path, codecs, target_dbtp=target_dbtp, safety_margin_db=codec_safety_margin_db
    )
    result["codec_preview"] = codec
    result["overall_status"] = max(
        (result["overall_status"], codec["status"]), key=lambda s: STATUS_RANK.get(s, 0)
    )
    return result
