...
MCP architecture: **#146** — split direction selected.  
Early read-only QA evidence track is separate from the v0.9 product MCP façade.  
Product MCP implementation remains deferred.
...
- v0.9 ComfyUI/runtime orchestration — #46, #55; includes the future Genre_test **product MCP façade** over stable APIs. The separate QA evidence-consumer track is cross-cutting engineering infrastructure and does not advance v0.9 product scope;
