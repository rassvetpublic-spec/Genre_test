...
function Ensure-Venv {
    ...
    if (-not (Test-Path -LiteralPath $venvPython)) {
        ...
        & $Runtime.Exe @invokeArgs | Out-Host
...
function Get-TorchState {
    ...
    if ($exitCode -eq 0 -and $markerLine) {
        ...
        return $state.Substring($marker.Length + 1)
    }
    return $null
}
...
function Ensure-Torch {
    ...
    if ($needsInstall) {
        if ($hasNvidia) {
            ...
            & $Python -m pip install --upgrade torch --index-url https://download.pytorch.org/whl/cu128 | Out-Host
        } else {
            Write-Host 'NVIDIA GPU not detected. Installing CPU PyTorch build...'
            & $Python -m pip install --upgrade torch --index-url https://download.pytorch.org/whl/cpu | Out-Host
        }
...
function Find-FFmpeg {
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path -LiteralPath $candidate -PathType Leaf)) { return [string]$candidate }
    }
    return $null
}
...
function Install-GenreTest {
    ...
    if (-not (Test-Path -LiteralPath $genreExe -PathType Leaf)) {
        throw "Genre_test executable is missing after installation: $genreExe"
    }
...
try {
    ...
    if ($genreExe -is [array]) {
        throw 'Internal bootstrap error: Install-GenreTest returned multiple values.'
    }
    Run-Diagnostics -GenreExe ([string]$genreExe)
